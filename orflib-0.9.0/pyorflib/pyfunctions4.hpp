...
static
PyObject*  pyOrfBermBSPDE(PyObject* pyDummy, PyObject* pyArgs)
{
PY_BEGIN;
  ...
  if (!PyArg_ParseTuple(pyArgs, "OOOOOOOOO", &pyPayoffType, &pyStrike, &pyTimesToExer,
    &pySpot, &pyDiscountCrv, &pyDivYield, &pyVolatility, &pyPdeParams, &pyAllResults))
    return NULL;
  ...
  double spot = asDouble(pySpot);
  double strike = asDouble(pyStrike);
  orf::Vector timesToExer = asVector(pyTimesToExer);
  ...
  // create the product
  orf::SPtrProduct spprod(new orf::BermudanCallPut(payoffType, strike, timesToExer));
  // create the PDE solver
  orf::Pde1DResults results;
  bool storeAllResults = true;
  orf::Pde1DSolver solver(spprod, spyc, spot, divYield, vol, results, storeAllResults);
