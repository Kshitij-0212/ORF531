...
def ppolySum(bkpoints1, values1, bkpoints2, values2, polyorder): 
    ...
    """
    return pyorflib.ppolySum(bkpoints1, values1, bkpoints2, values2, polyorder)
...
def polyBracket(coeffs, lolim, uplim, nsubs): 
    ...
    """
    return pyorflib.polyBracket(coeffs, lolim, uplim, nsubs)
...
def polySecant(coeffs, lolim, uplim, tol): 
    ...
    double
        the root
    """
    return pyorflib.polySecant(coeffs, lolim, uplim, tol)
...
def qFwdPrice(spot, timetoexp, intrate, divyield, assetvol, fxvol, correl):
    ...
    """
    return pyorflib.qFwdPrice(spot, timetoexp, intrate, divyield, assetvol, fxvol, correl)
...
def asianBasketBSMC(payofftype, strike, fixtimes, assetquantities, spots, discountcrv, divyields, 
    ...
    dictionary
        ...
        StdErr : Monte Carlo standard error
    """
    return pyorflib.asianBasketBSMC(payofftype, strike, fixtimes, assetquantities, spots, discountcrv, divyields, 
                                   volatilities, correlmat, mcparams, npaths)
...
def worstofDigiBSMC(payofftype, strike, timetofix, timetoexp, spots, discountcrv, divyields,
    ...
    strike : double
        strike price
    timetofix : double
        fixing time T1 in years
    timetoexp : double
        time to expiration/payment T2 in years
    spots : list(double) or 1D numpy array
        asset spot prices
    discountcrv : str
        discount yield curve name
    ...
    dictionary
        ...
        StdErr : Monte Carlo standard error
    """
    return pyorflib.worstofDigiBSMC(payofftype, strike, timetofix, timetoexp, spots, discountcrv, divyields,
                                   volatilities, correlmat, mcparams, npaths)
